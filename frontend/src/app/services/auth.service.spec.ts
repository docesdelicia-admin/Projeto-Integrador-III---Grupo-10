...
describe('AuthService', () => {
  ...
  const storageKey = 'docesdelicia.auth';
  ...
  it('realiza login e armazena dados do usuario', () => {
    ...
    service.login('admin@doces.com', '123456').subscribe((resposta) => {
      resultado = resposta;
    });
  ...
  it('envia do usuario com credenciais corretamente', () => {
    ...
    const req = httpMock.expectOne('/api/auth');
    expect(req.request.method).toBe('POST');
    expect(req.request.body).toEqual({ email: 'admin@doces.com', senha: '123456' });
  ...
  it('armazena sessao ao fazer login', () => {
    ...
    expect(window.sessionStorage.getItem(storageKey)).toContain('admin@doces.com');
  });

  it('lanca erro quando credenciais sao invalidas', () => {
    let erro: Error | undefined;
    service.login('admin@doces.com', 'errada').subscribe({
      error: (e: Error) => {
        erro = e;
      },
    });
  ...
  it('verifica se usuario nao eh admin', () => {
    ...
    service.login('operador@doces.com', '123456').subscribe();
  ...
  it('hidratra sessao do sessionStorage ao iniciar service', () => {
    ...
    expect(serviceHidratado.obterSessaoAutenticada()?.email).toBe('admin@doces.com');
  });
