...
describe('VitrinePage', () => {
  ...
  it('contem todos os titulos de categorias esperados', () => {
    const nomes = component.categorias.map((categoria) => categoria.titulo);
  ...
  it('cada categoria tem descricao', () => {
    const todasDescricoes = component.categorias.map((categoria) => categoria.descricao);
  ...
  it('todas as fotos sao data URLs SVG', () => {
    component.categorias.forEach((categoria) => {
      categoria.produto.fotos.forEach((foto) => {
  ...
  it('Bolos personalizados tem preco esperado', () => {
    const bolos = component.categorias.find(
      (categoria) => categoria.titulo === 'Bolos personalizados',
    );
    expect(bolos?.produto.preco).toBe(179.9);
  });

  it('Doces no pote tem preco esperado', () => {
    const doces = component.categorias.find((categoria) => categoria.titulo === 'Doces no pote');
